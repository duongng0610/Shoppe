...
public class PaymentService {
    ...
    public String createTransaction(HttpServletRequest request) throws AppException {
        ...
        if (status == TransactionStatus.SUCCESS) {
            ...
            eventPublisher.publishEvent(OrderPaid.builder().paymentDate(order.getCreatedAt()).client(userMapper.toDto(user)).sellerId(order.getSeller().getId()).variantId(order.getVariant().getId()));
        }
