...
public class CreateProductHelper {
  ...
  public Variant createVariant(
      VariantRequest variantRequest, Product product, Set<String> usedImageIds) {
    Variant variant =
        Variant.builder()
            .product(product)
            .price(variantRequest.getPrice())
            .quantity(variantRequest.getQuantity())
            .thumbnailId(variantRequest.getThumbnailId())
            .thumbnail(
                cloudinaryService.generateResizedUrl(variantRequest.getThumbnailId(), 300, 300))
            .build();

    usedImageIds.add(variantRequest.getThumbnailId());
