...
public class ProductService {
  ...
  public Product persistProduct(@Valid CreateProductRequest request) {
    ...
    usedImageIds.add(request.getThumbnailId());

    if (request.getExtraImageIds() != null && !request.getExtraImageIds().isEmpty()) {
      usedImageIds.addAll(request.getExtraImageIds());

      List<ProductExtraImage> productExtraImages =
          request.getExtraImageIds().stream()
              .map(
                  imageId -> {
                    ProductExtraImage image =
      ...
      product.setProductExtraImages(productExtraImages);
    }
    ...
    if (request.getHasVariant()) {
      if (request.getVariantRequests() != null && !request.getVariantRequests().isEmpty()) {
        ...
        for (int i = 0; i < request.getVariantRequests().size(); i++) {
          product.addVariant(
              createProductHelper.createVariant(
                  request.getVariantRequests().get(i), product, usedImageIds));
        }
      }
    } else {
  ...
  public ProductDetailResponse getProductDetail(String id) {
    ...
    eventPublisher.publishEvent(
        ProductViewed.builder().viewedAt(LocalDateTime.now()).productId(id).build());
