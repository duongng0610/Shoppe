...
public interface ProductRepository extends JpaRepository<Product, String> {
  ...
  @Query(
      value = "SELECT * FROM products p  WHERE p.status = 'APPROVED'  LIMIT :limit OFFSET :offset",
      nativeQuery = true)
  ...
  @Query(value = "UPDATE products p SET p.status = :status WHERE p.id IN :ids", nativeQuery = true)
  @Transactional
  @Modifying
