...
public class GetShoppingCartCountTest {
  ...
  void getShoppingCartCount_success() throws Exception {
    ...
    Mockito.when(shoppingCartService.getBasicShoppingCart()).thenReturn(response);

    mockMvc
        .perform(get("/client/shopping-cart/count"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.message").value("get shopping cart count successfully"))
  ...
  void getShoppingCartCount_nullData() throws Exception {
    ...
    Mockito.when(shoppingCartService.getBasicShoppingCart()).thenReturn(null);
  ...
  void getShoppingCartCount_serviceThrowsException() throws Exception {
    ...
    mockMvc.perform(get("/client/shopping-cart/count")).andExpect(status().isInternalServerError());
    ...
    mockMvc
        .perform(
            org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post(
                "/client/shopping-cart/count"))
        .andExpect(status().isInternalServerError());

    Mockito.verify(shoppingCartService, Mockito.never()).getBasicShoppingCart();
  }
  ...
  void getShoppingCartCount_wrongUrl() throws Exception {
    ...
    mockMvc
        .perform(get("/client/shopping-cart/counttt"))
        .andExpect(status().isInternalServerError());
  ...
  void getShoppingCartCount_calledTwice() throws Exception {
    ...
    mockMvc.perform(get("/client/shopping-cart/count")).andExpect(status().isOk());

    mockMvc.perform(get("/client/shopping-cart/count")).andExpect(status().isOk());

    Mockito.verify(shoppingCartService, Mockito.times(2)).getBasicShoppingCart();
  }
